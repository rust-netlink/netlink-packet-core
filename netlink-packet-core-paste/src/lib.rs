...
//! notoriously underpowered in that its concatenated identifiers can only refer
//! to existing items, they can never be used to define something new.
//!
...
//! These compose, so for example `$var:snake:upper` would give you
//! SCREAMING\_CASE.
//!
        ...
        match token {
            ...
            Some(TokenTree::Punct(punct)) => {
                lookbehind = match punct.as_char() {
                    ...
                    '#' => Lookbehind::Pound,
                    '!' if lookbehind == Lookbehind::Pound => {
                        Lookbehind::PoundBang
                    }
                    _ => Lookbehind::Other,
                };
...
fn is_single_interpolation_group(input: &TokenStream) -> bool {
    ...
    for tt in input.clone() {
        state = match (state, &tt) {
            ...
            (State::Init, TokenTree::Punct(punct))
                if punct.as_char() == '\'' =>
            {
                State::Apostrophe
            }
...
fn pasted_to_tokens(mut pasted: String, span: Span) -> Result<TokenStream> {
    ...
    #[cfg(not(no_literal_fromstr))]
    {
        ...
        if pasted.starts_with(|ch: char| ch.is_ascii_digit()) {
            let literal =
                match panic::catch_unwind(|| Literal::from_str(&pasted)) {
